...
import { CreateTaskInput, Task, TaskDifficultyLevel, createTask, updateTask } from '@/action/task.action';
...
const defaultFormData: Omit<CreateTaskInput, 'internship_id'> = {
    title: '',
    description: '',
    assigned_day: 1,
    resource_links: [],
...
export default function TaskFormModal({ isOpen, onClose, onSuccess, internshipId, maxDay, initialData }: TaskFormModalProps) {
    const [loading, setLoading] = useState(false);
    const [formData, setFormData] = useState<CreateTaskInput>({ ...defaultFormData, internship_id: internshipId });

    useEffect(() => {
        // Reset form when modal opens/closes
        if (!isOpen) {
            setFormData({ ...defaultFormData, internship_id: internshipId });
        }

        // Set initial data if editing
        if (isOpen && initialData) {
            const { id, created_at, ...taskData } = initialData;
            // Convert null values to undefined to match CreateTaskInput type
    ...
    }, [isOpen, initialData, internshipId]);
    ...
    const handleArrayInput = (field: keyof CreateTaskInput, value: string) => {
